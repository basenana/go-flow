...
func main() {
	...
	dep := ext.NewTaskDep()
	dep.RunOrder(task1.Name(), []string{task3.Name(), task4.Name()})
	dep.RunOrder(task2.Name(), []string{task3.Name(), task4.Name()})
	dep.RunOrder(task3.Name(), []string{task4.Name()})
	//dep.RunOrder(task4.Name(), []string{task3.Name()})

	// 3. new flow in storage
